...
function(a) {
	...
	function g(a, b) {
		if (b.trigger) {
			var c = $(a);
			j && clearTimeout(j), console.log(b.trigger.sends), b.trigger.sends && b.trigger.sends.length && $.each(b.trigger.sends, function(a, b) {
		...
		function b(a, b) {
			...
			if (c) {
				...
				if (a.css) {
					...
					}), "edit" != i.mode && "x" == ("" + a.type).charAt(0)) return e.append(c), e.find(".element-box").css({
	...
	i.addInterceptor(function(a, b, c) {
		...
		var b;
		if ("edit" == i.mode) b = document.createElement("div"), b.id = a.id, b.setAttribute("ctype", a.type), b.setAttribute("class", "element comp_paragraph editable-text"), a.content && (b.innerHTML = a.content), b.style.cursor = "default";
	...
	}), i.bindAfterRenderEvent("i", function(a, b) {
		if ("view" == i.mode) {
...
var eqxiu = function() {
		...
		var f, g, h, i, j, k, l, m, n, o = $(window),
			...
			C = function(a, b, c, e) {
				...
				function D() {
					if (Math.abs(w) > Math.abs(v) && completeEffect($(f.$currentPage))) if (w > 0) {
				...
				function F() {
					if (Math.abs(v) > Math.abs(w) && completeEffect($(f.$currentPage))) if (v > 0) {
						if (f._isDisableFlipPrevPage) return;
				...
				var Z = $("#con"),
					_ = !1;
				if (("8" == b || "9" == b) && (B = .7, A = 800), e.obj.property.hasOwnProperty("triggerLoop") || (e.obj.property.triggerLoop = !0), p = e.obj.property.triggerLoop, e.obj.property.autoFlip && (m = 1e3 * e.obj.property.autoFlipTime, d()), c[0].elements && c[0].elements.length) for (var aa = 0; aa < c[0].elements.length; aa++) eqxCommon.bindTrigger($("#inside_" + c[0].elements[aa].id), c[0].elements[aa]);
				!
				function() {
					...
					if (x && completeEffect($(f.$currentPage))) if (x = !1, f.$activePage) {
						f._isDisableFlipPage = !0;
						var c;
...
function(a, b) {
	...
	function eqFreepage(a, b, c) {
		...
		var f = '{"id":81395,"pageId":"","sceneId":"","num":1,"type":"4","isInput":0,"title":null,"content":null,"status":1,"css":{"borderRadius":"%","borderStyle":"solid","height":"136","zIndex":"1000","paddingTop":"0px","borderColor":"rgba(0,0,0,1)","boxShadow":"0 0px 0px #333333","color":"#000000","backgroundColor":"rgba(0,0,0,0)","borderWidth":"0px","width":"143","left":"93px","paddingBottom":"0px","top":"182px"},"properties":{"height":"100px","imgStyle":{"width":139,"height":136,"marginTop":"0px","marginLeft":"0px"},"width":"100px","src":"group1/M00/01/30/yq0JCFQpOR-AOULFAAFBPO1yzBQ984.jpg"}}',
			g = '{"id":81465,"pageId":"","sceneId":"","num":1,"type":"2","isInput":0,"title":null,"content":"<div style=\\"text-align: center;\\"><font color=\\"#ffffff\\" size=\\"3\\">击此处进行编辑</font></div>","status":1,"css":{"zIndex":"102","height":"65","width":"320","left":"0px","top":"70px"},"properties":{}}';
		if (ad = 1, a.obj.property.lastPageId) customLastPage = !0, $.ajax({
			...
			success: function(c) {
				if (!c.obj) {
					...
					var h = JSON.parse(e);
					return h.num = a.list.length + 1, h.elements[2].properties.src = a.obj.cover, h.elements[1].content = h.elements[1].content.replace(/场景名称/, a.obj.name), h.elements[1].properties.height = 65, h.elements[1].height = 65, void d(h, b, a)
				}
				c.obj.sceneId = a.obj.id;
				var i = JSON.parse(f);
				i.properties.src = a.obj.cover, c.obj.elements.push(i);
				var j = JSON.parse(g);
				j.content = j.content.replace(/击此处进行编辑/, a.obj.name), c.obj.elements.push(j);
				for (var k = 0; k < c.obj.elements.length; k++)"2" == c.obj.elements[k].type && /http:\/\/service.eqxiu.com\/eqs\/link/.test(c.obj.elements[k].content) && (console.log(c.obj.elements[k].content), c.obj.elements[k].content = c.obj.elements[k].content.replace(/;url=.*com"/, ";url=" + encodeURIComponent(redirectUrl) + '"'));
				d(c.obj, b, a)
			}
		});
		else {
			e = e.replace(/id=16060/, "id=" + a.obj.id);
			var h = JSON.parse(e);
			h.num = a.list.length + 1, h.elements[2].properties.src = a.obj.cover, h.elements[1].content = h.elements[1].content.replace(/场景名称/, a.obj.name), d(h, b, a)
		}
	}
	...
	function parsePage(pages, data) {
		...
		}, pageNum = 1; pageNum <= pages.length; pageNum++)
			if ($('<section class="main-page"><div class="m-img" id="page' + pageNum + '"></div></section>').appendTo(".nr"), 10 == pageMode && ($("#page" + pageNum).parent(".main-page").wrap('<div class="flip-mask" id="flip' + pageNum + '"></div>'), $(".main-page").css({
			width: $(".nr").width() + "px",
			height: $(".nr").height() + "px"
		...
		function(a) {
			...
			if (isMobile || $("img").on("dragstart", function(a) {
				...
				var pvUrl = PREFIX_S1_URL + "eqs/pv/" + data.obj.id;
